...
$app->post('/api/Ghost/getAccessToken', function ($request, $response) {
    ...
    $validateRes = $checkRequest->validate($request, ['accessToken','blogUrl','clientId','clientSecret','username','password']);
    ...
    $url .= $queryParam['userId'];
    unset($queryParam['userId']);
    $queryParam['grant_type'] = 'password';
    $queryParam['username'] = urlencode($queryParam['username']);

    try {

        $resp =  $client->request('POST', $url ,['query' => $queryParam] );
